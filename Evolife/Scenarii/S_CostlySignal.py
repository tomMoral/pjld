...
class Scenario(Default_Scenario):
	...
	def genemap(self):
		...
		for ControlGene in range(self.Parameter('ControlGeneNumber')):
			Gmap.append('SignalByQualityRange'+str(ControlGene))
		return [(G,0) for G in Gmap]	# default lengths
	...
	def new_agent(self, Child, parents):
		...
		SignalInvestment = Child.gene_relative_intensity('SignalByQualityRange'
