...
"""		EVOLIFE: Emergence of social classes
		...
		http://www.enst.fr/~icc/IC/Intranet/docs/Axtell_Epstein_Young.pdf
"""
	#=============================================================#
...
class Scenario(Default_Scenario):
	...
	def genemap(self):
		""" Defines the name of genes and their position on the DNA.
			(see Genetic_map.py)"""
		return [('Combative',8), ('Submissive', 8)]
	...
	def strategy(self, Indiv, PartnerExpectedAction):
		if PartnerExpectedAction > 50 + self.Parameter('SecurityMargin'):	# agressive partner
			if random.randint(0,100) < Indiv.gene_relative_intensity('Submissive'):
			...
			# if random.randint(0,100) < Indiv.gene_relative_intensity('Cooperative'):
				return PartnerExpectedAction
				# return 50
			# else:
				# return 100 - PartnerExpectedAction - self.Parameter('SecurityMargin')
			...
			if random.randint(0,100) < Indiv.gene_relative_intensity('Combative'):
				 # return PartnerExpectedAction
				 return 70
			else:
				# return 100 - PartnerExpectedAction - self.Parameter('SecurityMargin')
	...
	def update_positions(self, members, start_location):
		...
		for m in members:
			...
			# m.location = (m.gene_relative_intensity('Combative'), m.gene_relative_intensity('Cooperative'), colour)
