...
""" EVOLIFE: Scenario WernerDyer :
	see:	http://www.dessalles.fr/outgoing/P05013107.html
	and	 http://www.telecom-paristech.fr/~icc/IC/projects/P06121815.html
"""
	#=============================================================#
...
class Scenario(Default_Scenario):
	...
	def new_agent(self, child, parents):
		...
		if not self.female(child):
			...
			if bty < chh_th:
				child.Phene_value('Idle', MDNA[locus+7]*int((chh_th-bty)**1.3))
				child.Phene_value('Ad', bty + MDNA[locus+7]*int((chh_th-bty)**0.7))
			else:
...
##			return MaleDirection	#test
				#Bty consideration
		bty_th = int(''.join(map(str,FDNA[bty_locus])),base=2)
	...
	def life_game(self, members):
		...
		for Female in members:
			...
			if idle > 0:
				Female.Phene_value('Idle', idle-1)
				continue
	...
	def Move(self, Male, Direction):
		...
		if Partner is not None and self.female(Partner):
			...
			self.Ground.divert(Male, NewPosition)
		return
