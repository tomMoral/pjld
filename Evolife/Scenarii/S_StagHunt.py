...
class Scenario(Default_Scenario):
	...
	def interaction(self, indiv, others):
		...
		def signals(indiv,noisy=0):
			if noisy:
				return noise_add(indiv.gene_relative_intensity('signal'),self.Parameter('Noise')) > 50
			else:
		...
		def hunts_stag(indiv,signal):
			...
			"""
			if signal:
				return noise_add(indiv.gene_relative_intensity('hunt_if_1'),
							 self.Parameter('Noise')) > 50
			else:
				return noise_add(indiv.gene_relative_intensity('hunt_if_0'),
							 self.Parameter('Noise')) > 50
