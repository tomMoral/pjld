...
class Genome(DNA):
	...
	def update(self):
		...
		for locus in range(0,len(self.Scenario.GeneMap)):
			...
			self.genome[locus].intensity = self.read_DNA(b1,b2, coding = self.Scenario.Parameter('GeneCoding'))

	def gene_intensity(self, name):
		# absolute intensity addressed trough name
		return self.genome[self.Scenario.get_locus(name)].intensity

	def gene_relative_intensity(self, name):
		# relative intensity addressed through name
		return self.locus_relative_intensity(self.Scenario.get_locus(name))

	def locus_intensity(self, locus):
		# absolute intensity addressed trough locus
		return self.genome[locus].intensity

	def locus_relative_intensity(self, locus):
	...
	def signature(self):
		...
		return [self.locus_relative_intensity(locus[0]) for locus in enumerate(self.Scenario.GeneMap)]
