...
class ExpVector(object):
	...
	def __init__(self, Vect):
		self.vect = Vect
		self.size = len(Vect)
		self.values = sorted(set(Vect), key=lambda x: int(float(x)))
		self.variation = None
		self.majority = None
...
class ExpMatrix(object):
	...
	def File2Table(self, Filin):
		...
		LineOffset = 0
		if re.split("[\t;]+",strip(LinesTxt[0]))[0] == 'Evolife':
			LineOffset = 1
			self.Titles = re.split("[\t;]+",strip(LinesTxt[0]))
		self.Names = re.split("[\t;]+",strip(LinesTxt[LineOffset]))
		self.Lines = [re.split("[\t;]+",strip(L)) for L in LinesTxt[LineOffset+1:]]
	...
	def ColumnAnalysis(self, Parameter='', DataCol=[], verbose=True):
		...
		if AlmostConstant != [] and verbose:
			print 'Warning: those columns are not constant, but almost:',
			print ', '.join(AlmostConstant)

		RecentlyVariableParameters = [Col for Col in self.Parameters
										if self.Columns[Col].Variation() == 'Recently variable']
		if verbose:
			print "Recently variable parameters: ", RecentlyVariableParameters
	...
	def selectRelevantLines(self, X_parameter='', Y_parameter='',
		...
		if (X_parameter and X_parameter not in self.Names) \
		   or (Y_parameter and Y_parameter not in self.Names):
			print 'Available columns:', self.Names
		...
		try:
			...
			print 'please check parameter spelling: %s %s %s' % (X_parameter,Y_parameter, ' '.join(SideParameters.keys()))
			print '. . . Bye'
			sys.exit()
		...
		if X_parameter and Y_parameter:
			SelectedLines.sort(key=lambda x: (int(x[self.ColIndex(X_parameter)]),
											  int(x[self.ColIndex(Y_parameter)])))
		elif X_parameter:
			SelectedLines.sort(key=lambda x: (int(x[self.ColIndex(X_parameter)])))
...
def main():
	...
	try:
		...
		print "erreur dans les options : ", err
		print '. . . Bye'
		sys.exit(2)
