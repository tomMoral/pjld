	...
	def redraw(self):
		...
		for Curve in self.Curves:
			for Segment in Curve:
				#print Segment
				self.draw(Segment[0], Segment[1], ColorID=Curve.ID)
				pass

	def reframe(self,(x,y), Anticipation=False):
	...
	def grid(self):
		...
		for i in range(NbMailles+1):
			...
			self.addSimpleText(QString(str(int(i*self.scaleX/NbMailles)).center(3)),
						QFont(QString("Arial"), 8)).setPos(QPointF(self.LeftMargin+i*mailleX-7, self.H-self.BottomMargin+5))
			# legend on the y-axis
			self.addSimpleText(QString(str(int(i*self.scaleY/NbMailles)).rjust(3)),
						QFont(QString("Arial"), 8)).setPos(QPointF(self.LeftMargin-24, self.H - self.BottomMargin - i*mailleY-6))
...
##			self.removeItem(self.GraphicAgents[Agent])
	...
	def coordinates(self, Coord):
		...
		Start = Stroke(Coord[:4])
		if len(Coord) > 4:	End = Stroke(Coord[4:])
	...
	def create_graphic_agent(self, Position):
	...
	def restore_agent(self, Name):
		...
		self.move_agent(Name, self.positions.pop(Name) + self.segments.pop(Name))	# addition has been  redefined
