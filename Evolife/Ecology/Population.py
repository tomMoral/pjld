...
class Population:	
	...
	def __init__(self, Scenario, Observer):
		...
		self.best_score = 0
		if self.Scenario.Parameter('NumberOfGroups',Optional=True):
			nb_groups = self.Scenario.Parameter('NumberOfGroups')
			group_size = self.popSize // nb_groups
 		elif self.Scenario.Parameter('GroupMaxSize', Optional=True):
			# dynamic group size
			nb_groups = int(math.ceil(float(self.popSize) / min(self.popSize, self.Scenario.Parameter('GroupMaxSize'))))
			group_size = int(round(float(self.popSize) / nb_groups))	
		else:
			nb_groups = 1
			group_size = self.popSize
		while (nb_groups > 0):
	...
	def migration(self):
		...
		if len(self.groups) < 2 or self.Scenario.Parameter('MigrationRate', Optional=True) is None:
	...
	def group_splitting(self):
		...
		if not self.Scenario.Parameter('GroupMaxSize', Optional=True): return	# One single group
		##############
		...
		for gr in grps:
			if gr.size > self.Scenario.Parameter('GroupMaxSize'):
		...
		##############
		if self.Scenario.Parameter('GroupMinSize', Optional=True) is None: return	# No group minimum size
		grps = self.groups[:]   # copy of the list, necessary since 'groups' is modified within the loop
		for gr in grps:
...
##		MaxLives =  self.Scenario.Parameter('SelectionPressure')
	...
	def update(self, flagRanking = False):
		...
		for gr in self.groups:
			...
			grsize = gr.update_(flagRanking)
			if grsize == 0:
				toBeRemoved.append(gr)
			self.popSize += grsize
		for gr in toBeRemoved:
			self.groups.remove(gr)
		if self.popSize == 0:
	...
	def one_year(self):
		...
		try:
			...
			if self.Observer.Visible():
				self.statistics(self.Observer.Visible())	   # compute statistics before reproduction
...
class EvolifePopulation(Population):
	...
	def __init__(self, Scenario, Evolife_Obs):
		...
		if self.Scenario.Parameter('StartFromFile', Optional=True):
