...
class Group(object):
	...
	def update_(self, flagRanking = False):
		...
		if flagRanking:
			...
			if self.ranking != [] and self.ranking[0].score() == 0:
				# all scores are zero (assuming scores are positive)
...
class EvolifeGroup(Group):
	...
	def update_(self, flagRanking = False):
		...
		if flagRanking:
			self.Scenario.update_positions(self.ranking,self.location)
		else:
			self.Scenario.update_positions(self.members,self.location)
		return size
	...
	def reproduction(self):
		...
		for C in self.Scenario.couples(self.ranking):
			# making of the child
			child = self.createIndividual()	# blank individual
			if child:
	...
	def remove_(self, memberNbr):
		indiv = self.whoIs(memberNbr)
		indiv.detach()   # breaks solidarity links
