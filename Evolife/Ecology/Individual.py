...
class Individual:
	...
	def __init__(self, Scenario, ID=None, Newborn = True):
		...
		if not Newborn:
			...
			AgeMax = self.Scenario.Parameter('AgeMax', Optional=True)
			if AgeMax:	self.age = randint(1,AgeMax)
			else:	self.age = randint(1,100)
		else:	self.age = 0
		if ID:
			self.ID = ID
		else:
...
class EvolifeIndividual(Individual, Genome,Phenome,Alliances):
	...
	def __init__(self, Scenario, ID=None, Newborn=True):
		...
		Alliances.__init__(self, self.Scenario.None2Default('MaxGurus'), self.Scenario.None2Default('MaxFollowers'))
	...
	def observation(self, GroupExaminer):
		...
		GroupExaminer.store('Network',(self.ID,[T.ID for T in Alliances.signature(self)]), Numeric=False)
